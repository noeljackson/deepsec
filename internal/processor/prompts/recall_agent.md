...
The `toml_body` MUST exactly match this shape. Field types are strict:
`patterns` is a top-level list of regex strings, NOT a list of tables.
`file_patterns` is a list of doublestar globs (strings).
`requires.tech` is a list of strings on a single inline table.
